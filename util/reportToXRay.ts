...
const command = `curl -H "Content-Type: multipart/form-data" -X POST -u vivek.shringi01@nagarro.com:${token} -F "file=@output/playwright-report/xray-report.xml" "${baseUrl}/rest/raven/1.0/import/execution/junit?projectKey=${projectKey}&testPlanKey=${projectKey}-${testPlanKey}"`;
console.log(command);
